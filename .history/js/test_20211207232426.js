            ...
            $(".course-student-btn").click(function(){
                ...
                for(let i=0;i<course_array.length;i++){
                    if(course_array[i].name==courseName){
                        studentIds_array=course_array[i].studentIds
                    }
                }
                for(let i=0;i<studentIds_array.length;i++){
                    axios.get('http://localhost:3000/api/v1/user/getUserById/'+studentIds_array[i],{
                        headers: {
                            'Authorization' : `Bearer ${localStorage.toggled}`
                        }
                    })
                    .then(res =>{
                        ...
                        var td1_text = document.createTextNode("1");
                        var td2_text = document.createTextNode("course_students_array[0].fullName");
                        var td3_text = document.createTextNode("course_students_array[0].username");
                        var td4_text = document.createTextNode("course_students_array[0].email");
