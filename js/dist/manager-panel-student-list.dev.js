...
axios.get('http://localhost:3000/api/v1/user/my', {
  ...
  axios.get('http://localhost:3000/api/v1/user/list', {
    ...
    for (var i = 0; i < user_array.length; i = i + 1) {
    ...
    if (userlist_lenght == 0) {
      ...
      for (var _i = 0; _i < userlist_lenght; _i = _i + 1) {
        ...
        var img2 = document.createElement("img"); // var a=document.createElement("a");

        ul.className = "edit-delete";
        img1.className = "delete-student";
        img2.className = "edit-student";
        img1.src = "images/delete-icon.svg";
        img2.src = "images/edit-icon.svg"; // a.href="manager-panel-addStudent.html"

        ul.appendChild(li1);
        ul.appendChild(li2);
        li1.appendChild(img1);
        li2.appendChild(img2); // a.appendChild(img2)

        var td1_text = document.createTextNode(_i + 1);
        var td2_text = document.createTextNode(student_array[_i].username);
        var td3_text = document.createTextNode(student_array[_i].fullName); // Create a text node

        var td4_text = document.createTextNode(student_array[_i].email);
      ...
      $(document).ready(function () {
        ...
        $(".delete-student").click(function () {
          var tEmail = $(this).parent().parent().parent().parent().children(".student-email-td4").text();
          console.log(tEmail);

          for (var g = 0; g < userlist_lenght; g++) {
            if (student_array[g].email == tEmail) {
              console.log(student_array[g].id);
              studentId = student_array[g].id;
              break;
            }
          }
