...
$(document).ready(function () {
  axios.get('http://localhost:3000/api/v1/user/my', {
    ...
    axios.get('http://localhost:3000/api/v1/course/list', {
      ...
      console.log('get : ');
      var course_array = res.data.data; // console.log(course_array)

      if (course_array.length == 0) {
        document.getElementById("empty-list").innerHTML = "لیست  دروس  خالی است";
      } else {
        var _loop = function _loop(_i) {
          axios.get('http://localhost:3000/api/v1/user/getUserById/' + course_array[_i].profId, {
            headers: {
              'Authorization': "Bearer ".concat(localStorage.toggled)
            }
          }).then(function (res) {
            var teacher_fullName = res.data.data.fullName;
            ...
            var td2_text = document.createTextNode(course_array[_i].name);
            ...
            tr.appendChild(td4);
            tr.appendChild(td5);
            document.getElementById("list").appendChild(tr); // console.log(teachers_fullName)
        ...
        for (var _i = 0; _i < course_array.length; _i++) {
          _loop(_i);
        }
      }
      ...
      function students_list_modal() {
        ...
        if (studentIds_array.length == 0) {
          ...
          for (var a = 0; a < studentIds_array.length; a++) {
