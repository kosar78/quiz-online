    ...
    axios.get('http://localhost:3000/api/v1/question/getByExId/' + result.examId, {
      ...
      for (var i = 0; i < questions.length; i++) {
        ...
        for (var j = 0; j < questions[i].ques.options.length; j++) {
          ...
          if (j2 == answerChecked) {
            input.checked = true; // if(j2==questions[i].)
          }

          label.setAttribute("for", "option" + i2 + "-" + j2);
          var label_text = document.createTextNode(questions[i].ques.options[j]); // var ul3_li=document.createElement("li");
        ...
        $(".question-boxs-father").append(divquestionbox); // $(".answer-form").append(ul3)
